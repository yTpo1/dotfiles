...
http://list.ilocklist.com/?list=bt_templist
...
touch "$DIR/extras-$(date +%d-%b-%R).txt"

# download new blocklist
wget "${URLS[@]}" -O - | gunzip | LC_ALL=C sort -u > "$DIR/extras-$(date +%d-%b-%R).txt"b
#curl "${URLS[@]}" | gunzip | LC_ALL=C sort -u > "$DIR/extras-$(date +%d-%b-%R).txt"b

# directory in Ubuntu
#DIRA="/var/lib/transmission-daemon/info/blocklists"
# directory in Manjaro and Arch
DIRA="/var/lib/transmission/.config/transmission-daemon/blocklists"

sudo cp "$DIR/extras-$(date +%d-%b-%R).txtb" $DIRA
